...
class ModelExportYandexYml extends Model {
	...
	public function getProduct($allowed_categories, $blacklist, $out_of_stock_id, $vendor_required = true, $allowed_manufacturers = '') {
		$query = $this->db->query("SELECT
			...
			WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'"
				...
				.($blacklist ? " AND NOT(p.product_id IN (" . $this->db->escape($blacklist) . "))" : "")
