...
class ControllerFeedYandexYml extends Controller {
	...
	private function generateYml() {
		if ($this->config->get('yandex_yml_status')) {
			...
			$products = $this->model_export_yandex_yml->getProduct($allowed_categories, $blacklist, $out_of_stock_id, $vendor_required, $allowed_manufacturers);
				...
				if ($numpictures > 0) {
					if ($product['image'] && is_file(DIR_IMAGE . $product['image'])) {
						...
						if ($width_orig < 600 || $height_orig < 600)
							...
							$data['picture'] = array(HTTP_IMAGE . str_replace(' ', '%20', $product['image']));
					}
					//++++ Дополнительные изображения товара ++++
					if (isset($product_images[$product['product_id']])) {
						...
						foreach ($product_images[$product['product_id']] as $image) {
							...
							if ($width_orig < 600 || $height_orig < 600)
								...
								$data['picture'][] = HTTP_IMAGE . str_replace(' ', '%20', $image);
						}
					}
					//---- Дополнительные изображения товара ----
				}
