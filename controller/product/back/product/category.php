		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				$this->data['categories'][] = array(
					...
					'pic'	=> $this->model_tool_image->resize(($result['image']=='' ? 'no_image.jpg' : $result['image']), 80, 80)
				);
			}
			...
			foreach ($results as $result) {
				//Добавлена проверка для кадрирования изображений			
if($this->config->get('polianna_status')== 1){
				if ($result['image']) {
